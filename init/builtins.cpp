...
int do_mount_all(int nargs, char **args)
{
    ...
    if (ret == FS_MGR_MNTALL_DEV_NEEDS_ENCRYPTION) {
        ...
        init_property_set("ro.crypto.type", "block");
        ...
        init_property_set("ro.crypto.state", "encrypted");
        init_property_set("ro.crypto.type", "file");
        ...
        init_property_set("ro.crypto.state", "encrypted");
        init_property_set("ro.crypto.type", "file");
        init_property_set("vold.decrypt", "trigger_restart_min_framework");
    } else if (ret > 0) {
...
static void verity_update_property(fstab_rec *fstab, const char *mount_point, int mode, int status) {
    init_property_set(android::base::StringPrintf("partition.%s.verified", mount_point).c_str(),
...
static bool is_file_crypto() {
    ...
    __property_get("ro.crypto.type", prop_value);
